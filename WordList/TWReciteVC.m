...
{
    long displayType = [[NSUserDefaults standardUserDefaults] integerForKey:@"displayType"];
    UITableViewCell *cell = nil;
    if (indexPath.row < [self.words count]) {
        cell = [tableView dequeueReusableCellWithIdentifier:@"wordQuiz"];
        NSString * str = nil;
        NSManagedObject * wordObj = self.words[indexPath.row];
        switch (displayType) {
            case 0:
                str = [NSString stringWithFormat:@"%@\n%@", [wordObj valueForKey:@"word"], [wordObj valueForKey:@"meaning"]];
                break;
            case 1:
                str = [wordObj valueForKey:@"word"];
                break;
            case 2:
                str = [wordObj valueForKey:@"meaning"];
            default:
                break;
        }
